...
var g_toMove = colorWhite; // side to move, 0 or 8, 0 = black, 8 = white
...
function GetMoveSAN(move, validMoves){
    ...
    result=FormatMove(move);
	MakeMove(move);
	if (g_inCheck) result += GenerateValidMoves().length == 0 ? "#" : "+";
	UnmakeMove(move);
	return result;
}
...
function GetMoveFromString(moveString) {
 ...
 postMessage('info string busted! ->'+ moveString + " fen:" + GetFen());
}
...
function AllCutNode(ply, depth, beta, allowNull) {
    ...
    if ((g_nodeCount & 127) == 127) {
        if (g_timeout && ((new Date()).getTime() - g_startTime > g_timeout)) {
            // Time cutoff
            g_searchValid = false;
            return beta - 1;
        }
...
if(!fen)fen = 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1';
    ...
    if (chunks[3].indexOf('-') == -1) {
		...
		g_enPassentSquare =StrToSquare(chunks[3]);
    }
...
function Search(maxPly) {
    ...
    for (i = 1; i <= 99 && g_searchValid; i++) {
        ...
        if (g_hashTable[g_hashKeyLow & g_hashMask] != null) {
			...
			if(om!=bestMove){
			 numLegalMoves++;
			 postMessage('info currmove '+FormatMove(bestMove)+ ' currmovenumber ' + numLegalMoves);
			}
        }
            CallFinishPly(bestMove, value, (new Date()).getTime() - g_startTime, i);
			if(g_depthout && (i >= g_depthout))
				break;
			if(g_nodeout && (g_nodeCount + g_qNodeCount >= g_nodeout))
				break;
    }

if (bestMove != null) {
	MakeMove(bestMove);
	postMessage('bestmove ' +FormatMove(bestMove));
}
}

function CallFinishPly(bestMove, value, timeTaken, ply) {
var totalNodes = g_nodeCount + g_qNodeCount;
var pv = PVFromHash(bestMove, 15);
var sd = (pv.trim().split(' ')).length;
postMessage('info currmove '+FormatMove(bestMove)+' nodes '+totalNodes+' time '+timeTaken+' nps '+((totalNodes / (timeTaken / 1000)) | 0)+' depth ' + ply + ' seldepth ' + sd + ' score cp ' + (value >> 3) + ' pv' + pv);
}

function Start(d,t,n){
	g_timeout = t;
	g_nodeout = n;
	g_depthout = d;
	Search(d);
...
case 'go':
		...
		if (!t && !d && !n) {
			var wt = uci.getInt('wtime', 0);
		var bt = uci.getInt('btime', 0);
...
break;
case 'uci':
	postMessage('id name garbochess');
	postMessage('uciok');
break;
case 'ucinewgame':
	ResetGame();
