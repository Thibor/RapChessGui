...
namespace RapChessGui
{
	...
	public partial class FormChess : Form
	{
		...
		CGamer GamerD()
		{
			return rotateBoard ? CGamers.GamerBlack() : CGamers.GamerWhite();
		}

		CGamer GamerT()
		{
			return rotateBoard ? CGamers.GamerWhite() : CGamers.GamerBlack();
		}
		...
		bool TryMove(int s, int d)
		{
			...
			if (chess.IsValidMove(umo + "q", out _))
			{
				...
				Board.RenderBoard();
				RenderBoard();
				tlpPromotion.Dock = rotateBoard ^ chess.WhiteTurn ? DockStyle.Bottom : DockStyle.Top;
		...
		void TournamentBStart()
		{
			...
			p1.EngineName = CModeTournamentB.engine;
			p2.EngineName = CModeTournamentB.engine;
			p1.elo = b1.elo;
			p2.elo = b2.elo;
			p1.BookName = b1.name;
			p2.BookName = b2.name;
		...
		void TournamentBEnd(CGamer gw, CGamer gl, bool isDraw)
		{
			...
			int eloW = bw.Elo;
			int eloL = bl.Elo;
			bool f = CModeTournamentB.first == pw.BookName;
		...
		void TournamentEEnd(CGamer gw, CGamer gl, bool isDraw)
		{
			...
			int eloW = ew.Elo;
			int eloL = el.Elo;
			bool f = CModeTournamentE.first == pw.EngineName;
